...
b_chain.new_transaction("Lyuba", "Michelle B", 100)
# STEP 4 add the new block to the chain using the new_block() by passing a proof argument for each
b_chain.new_block(proof1)
# STEP 5 Print the chain with the new changes
print(b_chain.chain)
# STEP 6 Repeat steps 3-5 to record two more transactions
b_chain.new_transaction("Michelle B", "Brian", 200);
b_chain.new_block(proof2)
print(b_chain.chain)

b_chain.new_transaction("Brian", "Daiana", 300);
b_chain.new_block(proof3)
print(b_chain.chain)
